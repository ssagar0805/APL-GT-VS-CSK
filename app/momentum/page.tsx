import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { MomentumChart } from "@/components/dashboard/MomentumChart";

export default function MomentumPage() {
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black italic tracking-tighter text-slate-100">Momentum Engine</h1>
          <p className="text-slate-400 text-sm mt-1">Live structural momentum tracking and phase analysis.</p>
        </div>
        <div className="flex space-x-2">
          {['Past 10 Overs', 'Innings', 'Full Match'].map((period, i) => (
            <button key={period} className={`px-4 py-2 text-[10px] font-bold uppercase tracking-widest rounded-lg border ${i === 1 ? 'border-white/10 bg-white/5 text-blue-400' : 'border-transparent text-slate-500 hover:text-slate-300'}`}>
              {period}
            </button>
          ))}
        </div>
      </div>

      <section className="grid grid-cols-1 gap-6">
        <MomentumChart />
      </section>

      <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="border-white/10 bg-white/[0.03] rounded-2xl flex flex-col">
          <CardHeader className="border-b border-white/10 pb-4">
            <CardTitle className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-400">Pressure Index Swings</CardTitle>
          </CardHeader>
          <CardContent className="p-6">
             <div className="space-y-4">
                {[
                  { title: "Dot Ball Pressure", desc: "GT restricted 4 of last 5 overs to < 8 runs", impact: "+12%" },
                  { title: "Boundary Suppression", desc: "No boundaries in the last 15 deliveries", impact: "+18%" },
                  { title: "Wicket Impact", desc: "Loss of Gaikwad triggered severe structural disadvantage", impact: "-24%" }
                ].map((item, i) => (
                  <div key={i} className="flex justify-between items-center border-b border-white/5 pb-4 last:border-0 last:pb-0">
                    <div>
                      <h4 className="text-sm font-bold text-slate-200">{item.title}</h4>
                      <p className="text-xs text-slate-400 mt-1">{item.desc}</p>
                    </div>
                    <span className={`text-lg font-mono font-bold ${item.impact.startsWith('+') ? 'text-emerald-400' : 'text-red-400'}`}>
                      {item.impact}
                    </span>
                  </div>
                ))}
             </div>
          </CardContent>
        </Card>
        
        <Card className="border-white/10 bg-white/[0.03] rounded-2xl flex flex-col">
          <CardHeader className="border-b border-white/10 pb-4">
            <CardTitle className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-400">Phase Dominance</CardTitle>
          </CardHeader>
          <CardContent className="p-6 h-full flex items-center justify-center">
             <div className="text-center">
               <p className="text-slate-500 text-sm mb-4">Phase Dominance visualization loading...</p>
               <div className="w-48 h-48 rounded-full border-4 border-white/5 border-t-blue-500 animate-spin mx-auto opacity-50"></div>
             </div>
          </CardContent>
        </Card>
      </section>
    </div>
  );
}
