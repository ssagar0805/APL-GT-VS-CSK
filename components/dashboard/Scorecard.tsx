    ...
    <Card className="overflow-hidden border-white/10 bg-white/[0.03] rounded-2xl h-[330px]">
      <CardContent className="p-0 h-full flex flex-col justify-between">
        <div className="flex justify-between items-start p-6">
          <div>
            <Badge variant="outline" className="text-emerald-400 border-emerald-400/20 bg-emerald-400/10 hover:bg-emerald-400/20 mb-3 text-[10px] tracking-widest font-black uppercase">LIVE</Badge>
          ...
          <div className="text-right">
            ...
            <p className="text-xl font-mono font-bold text-white">{data.requiredRunRate}</p>
          </div>
        </div>

        <div className="grid grid-cols-2 divide-x divide-white/10 border-y border-white/10 bg-black/20 flex-1">
          <div className={`p-6 flex flex-col justify-center ${isHomeBatting ? 'bg-white/[0.02]' : 'opacity-40 grayscale -z-10'}`}>
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-bold uppercase tracking-widest text-slate-400">{data.teams.home.name}</span>
              {isHomeBatting && <span className="w-2 h-2 rounded-full bg-blue-500" />}
            </div>
            <div className="flex items-baseline space-x-2">
              <span className="text-5xl font-black tracking-tighter text-white">{data.teams.home.score}/{data.teams.home.wickets}</span>
            </div>
            <div className="mt-2 text-sm text-slate-400 font-mono">({data.teams.home.overs} ov) • CRR: {data.teams.home.runRate}</div>
          </div>
          
          <div className={`p-6 flex flex-col justify-center ${!isHomeBatting ? 'bg-white/[0.02]' : 'opacity-40 grayscale -z-10'}`}>
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-bold uppercase tracking-widest text-slate-400">{data.teams.away.name}</span>
              {!isHomeBatting && <span className="w-2 h-2 rounded-full bg-yellow-500" />}
            </div>
            <div className="flex items-baseline space-x-2">
              <span className="text-5xl font-black tracking-tighter text-white">{data.teams.away.score}/{data.teams.away.wickets}</span>
            </div>
            <div className="mt-2 text-sm text-slate-400 font-mono">({data.teams.away.overs} ov) • CRR: {data.teams.away.runRate}</div>
          </div>
        </div>
      </CardContent>
    </Card>
