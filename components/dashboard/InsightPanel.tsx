...
export function InsightPanel() {
  ...
  if (loading) {
     return (
       <Card className="col-span-1 h-full border-white/10 bg-gradient-to-b from-white/[0.03] to-transparent rounded-2xl flex flex-col justify-center items-center min-h-[350px]">
         <div className="w-8 h-8 rounded-full border-2 border-white/20 border-t-emerald-500 animate-spin mb-4"></div>
         <p className="text-xs uppercase tracking-widest text-slate-500 font-bold">Synthesizing Insights...</p>
       </Card>
     )
  }
  ...
  return (
    <Card className="col-span-1 h-full border-white/10 bg-gradient-to-b from-white/[0.03] to-transparent rounded-2xl flex flex-col min-h-[350px]">
      ...
      <CardContent className="p-0 flex-1 overflow-y-auto">
        <div className="divide-y divide-white/10">
          {insightsList.map((insight: any) => {
            ...
            return (
              <div key={insight.id} className="p-4 hover:bg-white/5 transition-colors group cursor-pointer">
                <div className="flex items-start">
                  <div className={`mt-1 mr-3 p-1.5 rounded bg-white/5 border border-white/10 ${insight.color}`}>
                  ...
                  <div>
                    <div className="flex items-center space-x-2 mb-1">
                      ...
                      <h4 className="text-xs font-bold text-slate-200 group-hover:text-white transition-colors">{insight.title}</h4>
                    </div>
